...
const int THREADPOOL_THREAD_NUM = 20;
...
int socket_bind_listen(int port)
{
    ...
    // 消除bind时"Address already in use"错误, 服务器在2MSL仍保留端口导致的问题
    int optval = 1;
    ...
    if(listen(listen_fd, LISTENQ) == -1)//最大监听队列长度
        return -1;
