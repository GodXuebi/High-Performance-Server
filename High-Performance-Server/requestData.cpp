...
#include <fcntl.h> //fcnti(fd,SETFL/GETFL,FLAG);
#include <queue> 
#include <cstdlib>
#include <sys/stat.h>
#include <sys/mman.h>
...
pthread_once_t MimeType::once_control = PTHREAD_ONCE_INIT; //Gurantee the MimeType only initalize one time.
...
void RequestData::reset()
{
    ...
    headers.clear();
    keep_alive = false;
    if (timer.lock())//get the shared_ptr from the week_ptr
    {
        shared_ptr<TimerNode> my_timer(timer.lock());
        my_timer->clearReq();//let the timers unbind with the Req
        timer.reset();//.reset() makes the reference counter minus one
    }
}
...
void RequestData::handleRequest()
{
    ...
    while (true)
    {
        ...
        if (state == STATE_PARSE_URI)
        {
            ...
            {
                LOG << "handleRequest flag == PARSE_URI_ERROR";
                isError = true;
                break;
            }
        }
        if (state == STATE_PARSE_HEADERS)
        {
            ...
            {
                LOG << "handleRequest flag == PARSE_HEADER_ERROR";
                isError = true;
                break;
            }
        ...
        if (state == STATE_RECV_BODY)
        {
            ...
            if (content.size() < content_length)
                continue;//In fact, the function will read again and directly come heres
            state = STATE_ANALYSIS;
        }
    ...
    //Only keep in alive can reach here,if not keep in alive, the 
    //Epoll::epoll_mod(fd, shared_from_this(), _epo_event) will not be called
    //Even if the other side continue to send message, epoll will not moniter this socket
    Epoll::add_timer(shared_from_this(), 500);
    __uint32_t _epo_event = EPOLLIN | EPOLLET | EPOLLONESHOT;
    //epoll_mod will add requestData again in the 
...
int RequestData::analysisRequest()
{
    if (method == METHOD_POST)
    {
        ...
        /* 
        //send back some information to other side
        string header;
        ...
        //deal with the receive data now
    ...
    */
    ...
    else if (method == METHOD_GET)
    {
        ...
        if(file_name == "hello")
        {
            ...
            body_buff += "<html><title>Hello World!</title>";
            body_buff += "<body bgcolor=\"ffffff\">";
            body_buff += "200" + short_msg;
            body_buff += "<hr><em> Xuebi's Web Server</em>\n</body></html>";
            ...
            size_t send_len = (size_t)writen(fd, send_buff, strlen(send_buff));
